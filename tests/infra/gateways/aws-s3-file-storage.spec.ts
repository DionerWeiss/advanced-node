...
describe('AwsS3FileStorage', () => {
  ...
  it('should config aws credentials on creation', () => {
    ...
    expect(config.update).toBeCalledWith({
      credentials: {
  ...
  describe('upload', () => {
    ...
    it('should return imageUrl', async () => {
      ...
      expect(imageUrl).toBe(`https://${bucket}.s3.amazonaws/${fileName}`)
    })

    it('should return encoded imageUrl', async () => {
      const imageUrl = await sut.upload({ fileName: 'any file name', file })

      expect(imageUrl).toBe(`https://${bucket}.s3.amazonaws/any%20file%20name`)
    })

    it('should rethrow if putObject throws', async () => {
      const error = new Error('upload_error')
      putObjectPromiseSpy.mockRejectedValueOnce(error)
      const promise = sut.upload({ fileName: 'any fileName', file })
